    ...
    for jSession=1:length(SESSIONS)
        ...
        try
            ...
            if ~(length(dtiQA)>1 && length(Stamper)==1 && length(MultiAtlas)==1)
                ...
                faM = loadniiorgz(faMname); faMinfo = niftiinfo(faMname);
                fa1 = loadniiorgz(fa1name); fa1info = niftiinfo(fa1name);
                fa2 = loadniiorgz(fa2name); fa2info = niftiinfo(fa2name);
                ...
                eve = loadniiorgz(eveName); eveinfo = niftiinfo(eveName);
                bc = loadniiorgz(brainColorName); bcinfo = niftiinfo(brainColorName);
